...
class chaining_data_store {
    ...
    struct header {
        ...
        constexpr static size_t large_size =
            sizeof(uint32_t) + sizeof(uint64_t);
        constexpr static size_t root_size =
        ...
        [[nodiscard]] inline std::vector<uint32_t> serialize() const {
            ...
            if (has_next) [[unlikely]] {
                ...
                if (total_size) {
                    buf.resize(5);
                    std::memcpy(buf.data() + 3, &total_size,
                                sizeof(total_size));
                }
            }
            return buf;
            ...
            if (has_next) [[unlikely]] {
                ...
                if (r2 != sizeof(next_ptr)) [[unlikely]] {
                    throw std::runtime_error(
                        "Could not read the header from file");
                }
                if (root_header) {
...
public:
    explicit chaining_data_store(chaining_data_store_config conf)
        ...
        for (const auto& entry :
             std::filesystem::directory_iterator(m_conf.directory)) {
            ...
            if (fd <= 0) [[unlikely]] {
                throw std::filesystem::filesystem_error(
                    "Could not open the files in the chaining data store root",
    ...
    index_type post_write(std::span<char> data) {
        ...
        for (const auto& partial_alloc : alloc) {
            if (lseek(partial_alloc.fd, partial_alloc.seek, SEEK_SET) !=
                partial_alloc.seek) [[unlikely]] {
                throw std::runtime_error(
                    "Could not seek to the allocated offset");
            }
            header h;
            h.has_next = index < (alloc.size() - 1);
            const auto header_size =
                header::get_header_size(h.has_next, index == 0);
            ...
            while (total_size < header_size) {
                const auto ws =
                    ::write(partial_alloc.fd, header_buffer.data() + total_size,
                            header_size - total_size);
                if (ws < 0) [[unlikely]] {
                    throw std::runtime_error("Could not write the header in "
                                             "the chaining data store");
                }
                total_size += ws;
            }

            for (size_t written = 0; written < h.size;
                 written +=
                 ::write(partial_alloc.fd, data.data() + offset + written,
                         h.size - written))
                ;
            offset += h.size;
    ...
    void revert_write() {
        throw std::runtime_error(
            "Revert in chaining data store not implemented");
    }
    ...
    unique_buffer<char> read(index_type index) {
        ...
        while (h.has_next) {
            ...
            if (::lseek(next_fd, next_seek, SEEK_SET) != next_seek)
                [[unlikely]] {
                throw std::runtime_error(
                    "Could not seek to the read position.");
            }
            ...
            do {
                const auto r =
                    ::read(next_fd, buf.data() + offset, h.size - tr);
                if (r == 0) [[unlikely]] {
                    break;
                }
                tr += r;
    ...
    void remove(index_type index) {
        ...
        for (size_t written = 0; written < h.size;
             written +=
             ::write(fd, buf, std::min(h.size - written, sizeof(buf))))
            ;
        m_free_spot_manager.push_free_spot(uint128_t(index),
                                           h.size + header_size);
    ...
    void sync() {
        ...
        for (const auto& modification : m_modified_files) {
            ...
            if (::lseek(fd, 0, SEEK_SET) != 0) [[unlikely]] {
                throw std::runtime_error(
                    "Could not seek to the first position.");
            }
    ...
    [[nodiscard]] size_type get_used_space() const noexcept { return m_used; }
...
private:
    ...
    get_file_offset_pair(index_type pointer) const {
    ...
    [[nodiscard]] size_type fetch_used_space() const noexcept {
        const auto prev_files_data_size =
            m_conf.max_file_size * (m_open_files.size() - 1);
        return prev_files_data_size + m_last_file_data_end -
               m_free_spot_manager.total_free_spots().get_low();
    }
    ...
    alloc_t allocate(std::size_t size) {
        ...
        while (free_spot.has_value() and required_size > 0) {
            const auto [fd, seek] =
                get_file_offset_pair(free_spot->pointer.get_low());
            partial_size = std::min(required_size, free_spot->size);
            alloc.emplace_back(fd, seek, partial_size,
                               free_spot->pointer.get_low());
        ...
        if (free_spot.has_value() and partial_size < free_spot->size)
            [[unlikely]] {
            m_free_spot_manager.note_free_spot(free_spot->pointer +
                                                   partial_size,
        ...
        while (required_size > 0) {
            partial_size =
                std::min({m_conf.max_chunk_size, required_size,
                          m_conf.max_file_size - last_file_data_end});

            if (partial_size == 0) [[unlikely]] {
                // TODO revert the new file if IO not successful
                m_modified_files.insert_or_assign(m_last_fd,
                                                  last_file_data_end);
                ...
                partial_size =
                    std::min({m_conf.max_chunk_size, required_size,
                              m_conf.max_file_size - m_last_file_data_end});
            ...
            if (last_file_data_end + partial_size <= m_last_file_size)
                [[likely]] { // write in last file
                alloc.emplace_back(m_last_fd, last_file_data_end, partial_size,
                                   m_last_file_offset + last_file_data_end);

            } else if (m_last_file_size <
                       m_conf.max_file_size) { // double the file size
                auto new_file_size = m_last_file_size << 1;
                while (new_file_size - last_file_data_end < partial_size and
                       new_file_size < m_conf.max_file_size) {
                    new_file_size <<= 1;
                }
                const int rc =
                    ftruncate(m_last_fd, static_cast<long>(new_file_size));
                if (rc != 0) [[unlikely]] {
                    throw std::filesystem::filesystem_error(
                        "Could not extend file in the data store",
                        std::error_code(errno, std::system_category()));
                }
    ...
    int add_new_file(const index_type& offset, long file_size) {
        ...
        const int fd =
            open(file_path.c_str(), O_RDWR | O_CREAT, S_IWUSR | S_IRUSR);
        if (fd <= 0) [[unlikely]] {
            throw std::filesystem::filesystem_error(
    ...
    [[nodiscard]] static std::pair<int, index_type>
    ...
    [[nodiscard]] std::string get_name(const index_type offset) const {
        return "data_" + std::to_string(m_data_id) + "_" +
