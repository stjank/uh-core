...
perform_for_address(
    ...
    co_return co_await run_for_all<R, std::size_t, address_info>(
        ioc,
        [&storages, &func](std::size_t id, const address_info& addr_info)
            -> coro<R> { co_return co_await func(storages[id], addr_info); },
        addr_map);
}
