...
void free_spot_manager::sync() { m_file.sync(); }

free_spot_manager::~free_spot_manager() { sync(); }

std::fstream free_spot_manager::open_file() {
    if (std::filesystem::exists(m_hole_log)) {
        std::fstream file{m_hole_log,
                          std::ios::binary | std::ios::in | std::ios::out};
        if (!file) {
            throw std::filesystem::filesystem_error(
                "Could not open/create the log file in the data store root",
        ...
        std::fstream file{m_hole_log, std::ios::binary | std::ios::in |
...
void free_spot_manager::shift_forward() {
    ...
    if (offset > 0 and offset < m_end_pos) {
        ...
        read_size(m_file, buffer_size - ELEMENT_SIZE,
                  buffer.get() + ELEMENT_SIZE);
