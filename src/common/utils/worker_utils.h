...
struct worker_utils {
    ...
    static coro<std::invoke_result_t<Func, Args...>>
    post_in_workers(boost::asio::thread_pool& workers,
    ...
    static coro<void> io_thread_acquire_messenger_and_post_in_io_threads(
        boost::asio::thread_pool& workers, boost::asio::io_context& ioc,
        std::shared_ptr<client> cl, Func func) {

        auto f = [](auto& cl) { return cl->acquire_messenger(); };
        auto m =
            co_await post_in_workers(workers, ioc, std::bind(f, std::ref(cl)));
    ...
    static void broadcast_from_worker_in_io_threads(
        const std::vector<std::shared_ptr<client>>& nodes,
        boost::asio::io_context& ioc, Func func) {
        ...
        for (auto& n : nodes) {
            ...
            futures.emplace_back(boost::asio::co_spawn(
                ioc, func(std::move(m), i++), boost::asio::use_future));
        }
    ...
    static coro<void> broadcast_from_io_thread_in_io_threads(
        const std::vector<std::shared_ptr<client>>& nodes,
        boost::asio::io_context& ioc, boost::asio::thread_pool& workers,
        Func func) {
        auto f = [](const auto& nodes, auto& ioc, auto func) {
            broadcast_from_worker_in_io_threads(nodes, ioc, std::move(func));
        };

        co_await post_in_workers(
            workers, ioc,
            std::bind(f, std::cref(nodes), std::ref(ioc), std::move(func)));
    }
};
