        ...
        while (offset < file_size)
        {
            ...
            switch (op_fe.first) {
                ...
                case set_operation::REMOVE:
                    //set.erase({op_fe.second.pointer, op_fe.second.size, op_fe.second.prefix, storage});
                    break;
                default:
...
private:
    ...
    [[nodiscard]] std::pair <set_operation, entry> deserialize () const {
        char buf [sizeof (entry)];
        if (const auto rc = ::read(m_log_file, buf, sizeof buf); rc != sizeof buf) [[unlikely]] {
            perror ("Read error");
            throw std::runtime_error ("Could not read the set element from the set log file");
        }
        entry f;
        size_t offset = sizeof set_operation::INSERT;
    ...
    template <typename Set>
    void recreate (Set&& set) {
        const auto new_file_path = m_log_path.parent_path() / "_set_logger_tmp_file_new";
        const auto old_file_tmp_path = m_log_path.parent_path() / "_set_logger_tmp_file_original";
        try {
