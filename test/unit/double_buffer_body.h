...
struct double_buffer_body {
    ...
    struct value_type {
        /** A pointer to a contiguous area of memory of @ref rsize octets, else
           `nullptr`.

            @par Only for Parsing
            ...
            by `rdata` having `rsize` octets of valid storage. After
            octets are stored, the `rdata` and `rsize` members are
            adjusted: `rdata` is incremented to point to the next
            octet after the rdata written, while `rsize` is decremented
            to reflect the remaining space at the memory location
            pointed to by `rdata`.
        */
        void* rdata = nullptr;

        /** The number of octets in the buffer pointed to by @ref rdata.

            @par Only for Parsing

            The value of this field will be decremented during parsing
            to indicate the number of remaining free octets in the
            buffer pointed to by `rdata`. When it reaches zero, the
            parser will return @ref error::need_buffer, indicating to
            the caller that the values of `rdata` and `rsize` should be
            updated to point to a new memory buffer.
        */
        std::size_t rsize = 0;

        /** A pointer to a contiguous area of memory of @ref wsize octets, else
           `nullptr`.

            @par Only for Serializing
        ...
        */
        ...
        /** The number of octets in the buffer pointed to by @ref wdata.
            ...
            If `wdata` is `nullptr` during serialization, this value
            is ignored. Otherwise, it represents the number of valid
            body octets pointed to by `wdata`.
        */
        std::size_t wsize = 0;
        ...
        /** `true` if this is not the last buffer.
            ...
            If this is `true` and `wdata` is `nullptr`, the error
            @ref error::need_buffer will be returned from @ref serializer::get
    ...
    public:
        ...
        std::size_t put(ConstBufferSequence const& buffers, error_code& ec) {
            ...
            auto const bytes_transferred = net::buffer_copy(
                net::buffer(body_.rdata, body_.rsize), buffers);
            body_.rdata = static_cast<char*>(body_.rdata) + bytes_transferred;
            body_.rsize -= bytes_transferred;
    ...
    public:
        ...
        get(error_code& ec) {
            ...
            if (body_.wdata) {
                ...
                toggle_ = true;
                return {
                    {const_buffers_type{body_.wdata, body_.wsize}, body_.more}};
            }
            if (body_.more) {
                BOOST_BEAST_ASSIGN_EC(ec, error::need_buffer);
            } else
...
#if !BOOST_BEAST_DOXYGEN
// operator<< is not supported for double_buffer_body
template <bool isRequest, class Fields>
std::ostream&
operator<<(std::ostream& os,
           message<isRequest, double_buffer_body, Fields> const& msg) = delete;
#endif
