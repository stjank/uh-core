...
BOOST_AUTO_TEST_CASE(throws_for_missing_parity_shards_for_ec_group) {
    static constexpr const char* json_literal =
        R"([{"id":0,"type":"ERASURE_CODING","storages":3,"data_shards":2,"chunk_size_kib":1024}])";
...
BOOST_AUTO_TEST_CASE(deserializes_ec_group) {
    static constexpr const char* json_literal =
        R"([{"id":0,"type":"ERASURE_CODING","storages":3,"data_shards":2,"parity_shards":1,"chunk_size_kib":1024}])";
...
BOOST_AUTO_TEST_CASE(parses_json_string_to_license) {
    ...
    BOOST_TEST(sut.configs[0].chunk_size_kib == 1024);
