...
#include "common/serialization_common.h"
...
class EnDecoder {
    ...
    enum fundamental {
        char_type, short_type, int_type, long_type, float_type, double_type, long_double_type, string_type
    };
...
public:
    ...
    OutType encode(Args input, Size...length) {
        ...
        type_byte = 0;
        if constexpr ((std::is_same_v<unsigned char *, Args> or std::is_same_v<char *, Args>) and
                      sizeof...(Size) == 0) {
            BOOST_STATIC_ASSERT_MSG("Size not given to encoder on type " + type_name<Args>() + " !");
            return std::vector<unsigned char>{};
        }
        if constexpr (sizeof...(Size) > 1 or (sizeof...(Size) == 1 and (not(std::is_same_v<unsigned char *, Args> or
                                                                            std::is_same_v<char *, Args>) or
                                                                        SequentialContainer<Args> or
                                                                        std::is_same_v<Args, std::istringstream> or
                                                                        std::is_same_v<Args, std::istream> or
                                                                        std::is_same_v<Args, std::stringstream> or
                                                                        std::is_same_v<Args, std::fstream>))) {
            BOOST_STATIC_ASSERT_MSG("Too many size variables given on type " + type_name<Args>() + " !");
            return std::vector<unsigned char>{};
        }
        if constexpr (std::is_same_v<Args, std::istringstream> or std::is_same_v<Args, std::istream> or
                      std::is_same_v<Args, std::stringstream> or std::is_same_v<Args, std::fstream>) {
            char c;
        ...
        if constexpr (std::is_same_v<long double, Args>) {
            ...
            if constexpr (SequentialContainer<Args> and
                          not(std::is_same_v<Args, unsigned char *> or std::is_same_v<Args, char *> or
                              std::is_same_v<Args, bool *>)) {
                using innerType = boost::mp11::mp_first<Args>;
                if constexpr (!ArithmeticFundamental<innerType>) {
                    BOOST_STATIC_ASSERT_MSG(
                            "The sequential input container to be encoded did not contain a fundamental arithmetic within on type " +
                            type_name<Args>() + " !");
                }
                if constexpr (VectorStandard<Args> and
                              (std::is_same_v<innerType, unsigned char> or std::is_same_v<innerType, char> or
                               std::is_same_v<innerType, bool>)) {
                    ...
                    for (const auto &i: input) {
                        ...
                        std::vector<unsigned char> singleBuf{
                                readBuf + std::is_same_v<long double, innerType> ? 2 : 0,
                                readBuf + sizeof(innerType)};
                ...
                if constexpr ((std::is_same_v<bool, Args> or std::is_same_v<char, Args> or
                               std::is_same_v<unsigned char, Args>)) {
                    ...
                    if constexpr (std::is_same_v<short, Args> or std::is_same_v<unsigned short, Args>) {
                        ...
                        if constexpr (std::is_same_v<int, Args> or std::is_same_v<unsigned int, Args>) {
                            ...
                            if constexpr (std::is_same_v<long int, Args> or
                                          std::is_same_v<unsigned long int, Args>) {
...
//                                                delete[] internalBuffer;
                                                ...
                                                if constexpr (std::is_same_v<unsigned char *, Args>) {
                                                    mem_size = static_cast<std::size_t>(std::forward<Size...>(
                                                            length...));
                                                    ...
                                                    if constexpr (std::is_same_v<char *, Args>) {
                                                        ...
                                                        if constexpr (std::is_same_v<std::string, Args>) {
                                                            ...
                                                            BOOST_STATIC_ASSERT_MSG("Type " + type_name<Args>() +
                                                                                    " was unknown to encoder and decoder!");
                                                            return std::vector<unsigned char>{};
                                                        }
                                                    }
                                                }
                                            ...
                                            std::memcpy(mem_size_convert.data(),
                                                        reinterpret_cast<unsigned char *>(&mem_size),
                                                        sizeof(mem_size));
                                            for (unsigned char i = 0; i < byte_count; i++) {
                                                size_representation.insert(size_representation.begin(),
                                                                           mem_size_convert[i]);
                                            }
                                        }
                                    }
                ...
                if constexpr (!(String<Args> or std::is_same_v<char *, Args> or
                                std::is_same_v<unsigned char *, Args> or std::is_same_v<Args, std::istringstream> or
                                std::is_same_v<Args, std::istream> or std::is_same_v<Args, std::stringstream> or
                                std::is_same_v<Args, std::fstream> or SequentialContainer<Args>)) {
    ...
    OutType get_encoding() {
        ...
        output.insert(output.end(), std::get<1>(encode_tuple),
                      std::get<1>(encode_tuple) + std::get<0>(encode_tuple));
        return output;
    }
    ...
    std::tuple<OutType, Args...> decoder(SomeStream &in, Args...args) {
        if constexpr (not(SequentialContainer<SomeStream> or std::is_same_v<SomeStream, std::istringstream> or
                          std::is_same_v<SomeStream, std::istream> or
                          std::is_same_v<SomeStream, std::stringstream> or
                          std::is_same_v<SomeStream, std::fstream>)) {
        ...
        if constexpr (SequentialContainer<SomeStream> and sizeof...(Args) != 1) {
            BOOST_STATIC_ASSERT_MSG(
                    "Wrong number of step iterators given due to inserting some sequential container!");
        }
        if constexpr (SequentialContainer<SomeStream> and
                      not std::is_same_v<boost::mp11::mp_first<SomeStream>, unsigned char>) {
        ...
        if (std::signed_integral<OutType> and !get_sign()) {
            DEBUG << "The Type " + std::string(type_name<OutType>()) + " was not signed! Decoding error!"
                  << std::endl;
            std::exit(EXIT_FAILURE);
        }

        if (std::unsigned_integral<OutType> and get_sign()) {
            DEBUG << "The Type " + std::string(type_name<OutType>()) + " was not unsigned! Decoding error!"
                  << std::endl;
            std::exit(EXIT_FAILURE);
        }

        fundamental As_type;
        if constexpr (std::is_same_v<bool, OutType> or std::is_same_v<char, OutType> or
                      std::is_same_v<unsigned char, OutType>) {
            As_type = char_type;
        } else {
        ...
        if (As_type != foundType) {
            DEBUG << "The saved type " + std::string(type_name<OutType>()) +
                     " of the encoding did not match the output wish!" << std::endl;
            std::exit(EXIT_FAILURE);
        }
        ...
        if constexpr (ArithmeticFundamental<OutType>) {
            ...
            if (sizeof(OutType) != mem_size)[[unlikely]] {
                DEBUG << "The type " + std::string(type_name<OutType>()) +
                         " that should be returned by the decoder did not fit the saved type!" << std::endl;
                std::exit(EXIT_FAILURE);
            } else[[likely]] {
            ...
            if constexpr (std::is_same_v<OutType, unsigned char *> or std::is_same_v<OutType, char *> or
                          std::is_same_v<OutType, bool *>) {
                ...
                if constexpr (String<OutType>) {
                    ...
                    if constexpr (SequentialContainer<OutType>) {
                        ...
                        if constexpr (!ArithmeticFundamental<innerType>) {
                            BOOST_STATIC_ASSERT_MSG("Error: The inner type " + std::string(type_name<OutType>()) +
                                                    " of the sequential container was not a fundamental type!");
                        }
                        ...
                        if constexpr (std::is_same_v<long double, innerType>) {
                            if (mem_size % 10 != 0)[[unlikely]] {
                                DEBUG << "The inner type " + std::string(type_name<OutType>()) +
                                         " of the expected container " + std::string(type_name<OutType>()) +
                                         " must be wrong since the modulo check of length failed while decoding!"
                                      << std::endl;
                                std::exit(EXIT_FAILURE);
                            }
                        } else {
        ...
        ERROR << "The type " + std::string(type_name<OutType>()) + " to be returned was unknown by the encoder!"
              << std::endl;
        if constexpr (SequentialContainer<SomeStream>) {
            return std::make_tuple(OutType{}, std::get<1>(input_tup));
        } else {
